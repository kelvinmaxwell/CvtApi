...
public interface QuotationRepository extends JpaRepository<Quotations,Long> {
    ...
    @Query(value="SELECT garages.name as garageName,vehicle_details.vehicle_registration_plate as vehicleRegistration,quotations.status,quotations.created_at  FROM quotations inner join garages on garages.id=quotations.garage_id inner join vehicle_details on vehicle_details.vehicle_id=quotations.vehicle_id where quotations.customer_id=?1", nativeQuery = true)
