...
public interface ForumsRepository    extends JpaRepository<Forums,Long> {
    ...
    @Query(value="select forums.name as forumName,forums.id,forums.summary,ifnull((select count(*) from forum_users where forum_id=forums.id),0)as followers,forums.vehicle_model,forums.image from forums where forums.is_private=0 and forums.id NOT IN (select forum_users.forum_id from forum_users where user_id=?1)", nativeQuery = true)
